...
test('hash function monkey patching', () => {
  ...
  for (const [key, value] of objects) {
    expect(map.has(key)).toStrictEqual(false);
    expect(map.has(keyHash(key))).toStrictEqual(false);
    expect(map.set(key, value)).toStrictEqual(map);
    expect(map.has(key)).toStrictEqual(true);
    expect(map.get(key)).toStrictEqual(value);
  }
  ...
  expect(map.size).toStrictEqual(objects.length * 3);
});
...
test('equal function monkey patching', () => {
  ...
  for (const [key, value] of objects) {
    ...
    const isInMap = actualHashPairs.some(([, [k]]) => k === key);
    expect(map.has(key)).toStrictEqual(isInMap);
    expect(map.has(keyHash(key))).toStrictEqual(false);
    expect(map.has(cloned)).toStrictEqual(false);
    if (!isInMap) {
      expect(map.set(key, value)).toStrictEqual(map);
      expect(map.has(cloned)).toStrictEqual(false);
      expect(map.has(key)).toStrictEqual(true);
  ...
  const logs = [];
  map.keysEqual = anotherEqual.bind(logs);
  for (const [key, value] of objects) {
    expect(map.has(key)).toStrictEqual(true);
    expect(map.has(clone(key))).toStrictEqual(true);
  }
  expect(map.size * 2).toStrictEqual(logs.length);
});